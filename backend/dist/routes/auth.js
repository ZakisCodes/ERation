...
router.post('/send-otp', async (req, res) => {
    try {
        ...
        if (!rationCardId || !mobileNumber) {
            return res.status(400).json({ message: 'Ration Card ID and Mobile Number are required' });
        }
        // Check if ration card exists
        const rationCard = await new Promise((resolve, reject) => {
            init_1.db.get('SELECT * FROM ration_cards WHERE id = ?', [rationCardId], (err, row) => {
                if (err)
                    reject(err);
                else
                    resolve(row);
        ...
        await new Promise((resolve, reject) => {
            init_1.db.run('INSERT INTO otp_verifications (id, ration_card_id, mobile_number, otp, expires_at) VALUES (?, ?, ?, ?, ?)', [otpId, rationCardId, mobileNumber, otp, expiresAt.toISOString()], (err) => {
                if (err)
                    reject(err);
                else
                    resolve(true);
...
router.post('/verify-otp', async (req, res) => {
    try {
        ...
        if (!otp || !rationCardId || !mobileNumber) {
            return res.status(400).json({ message: 'OTP, Ration Card ID and Mobile Number are required' });
        }
        // Check OTP validity
        const otpRecord = await new Promise((resolve, reject) => {
            init_1.db.get('SELECT * FROM otp_verifications WHERE ration_card_id = ? AND mobile_number = ? AND otp = ? AND is_used = FALSE AND expires_at > datetime("now") ORDER BY created_at DESC LIMIT 1', [rationCardId, mobileNumber, otp], (err, row) => {
                if (err)
                    reject(err);
                else
                    resolve(row);
        ...
        // Mark OTP as used
        await new Promise((resolve, reject) => {
            init_1.db.run('UPDATE otp_verifications SET is_used = TRUE WHERE id = ?', [otpRecord.id], (err) => {
                if (err)
                    reject(err);
                else
                    resolve(true);
        ...
        // Generate JWT token
        const token = jsonwebtoken_1.default.sign({ rationCardId, mobileNumber }, process.env.JWT_SECRET || 'your-secret-key', { expiresIn: '24h' });
        res.json({
...
router.get('/family-members/:rationCardId', async (req, res) => {
    try {
        ...
        const members = await new Promise((resolve, reject) => {
            init_1.db.all('SELECT * FROM family_members WHERE ration_card_id = ? ORDER BY created_at', [rationCardId], (err, rows) => {
                if (err)
                    reject(err);
                else
                    resolve(rows);
...
router.post('/verify-id', async (req, res) => {
    try {
        ...
        await new Promise((resolve, reject) => {
            init_1.db.run('UPDATE family_members SET government_id = ?, government_id_type = ?, verification_status = "verified", is_verified = TRUE WHERE id = ?', [governmentIdNumber, governmentIdType, memberId], (err) => {
                if (err)
                    reject(err);
                else
                    resolve(true);
...
router.post('/complete-verification', async (req, res) => {
    try {
        ...
        await new Promise((resolve, reject) => {
            init_1.db.run('UPDATE family_members SET verification_status = "verified", is_verified = TRUE WHERE id = ?', [memberId], (err) => {
                if (err)
                    reject(err);
                else
                    resolve(true);
